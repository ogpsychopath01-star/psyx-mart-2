import { Message, EmbedBuilder, PermissionFlagsBits, TextChannel, ButtonBuilder, ActionRowBuilder, ButtonStyle } from 'discord.js';
...
import { isMaintenanceMode } from '../utils/state.js';
...
export default function registerMessageCreate(client: BotClient) {
  client.on('messageCreate', async (message: Message) => {
    ...
    if (message.guild && !isBotOwner(message.author.id)) {
      if (isCommandDisabled(message.guild.id, command.name, command.category)) {
        message.reply({
          embeds: [new EmbedBuilder()
            .setColor(COLORS.warning)
            .setTitle('🚫 Command Disabled')
            .setDescription(`The command \`${PREFIX}${command.name}\` is currently **disabled** in this server.`)
            .setFooter(BOT_FOOTER)
            .setTimestamp()
          ]
        }).catch(() => {});
