...
import {
  getAutomodSetting, addWarning, getAutomodWhitelist,
  getAutomodUserWhitelist, getAutomodRoleWhitelist, getLogChannel,
} from '../database.js';
...
export async function handleAutomod(message: Message): Promise<boolean> {
  ...
  // Whitelist — users exempt from all automod
  const userWhitelist = getAutomodUserWhitelist(guildId);
  if (userWhitelist.includes(message.author.id)) return false;

  // Whitelist — roles exempt from all automod
  const roleWhitelist = getAutomodRoleWhitelist(guildId);
  if (member.roles.cache.some(r => roleWhitelist.includes(r.id))) return false;
