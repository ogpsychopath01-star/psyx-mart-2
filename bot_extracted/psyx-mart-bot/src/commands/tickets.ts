...
const ticketCommands: BotCommand[] = [
  ...
  // ── TICKET SETUP ──────────────────────────────────────────────────────────
  {
    ...
    category: 'Tickets',
    usage: 'ticketsetup <#channel|channelID> [categoryID]',
    ...
    async execute(message, args) {
      ...
      if (!channel)
        return message.reply({ embeds: [errorEmbed('Missing Channel', 'Mention the channel or provide its **Channel ID** where the ticket panel should be posted.\nExample: `!ticketsetup #tickets` or `!ticketsetup 1234567890123456789`')] });

      // Category is always args[1] (or args[0] when channel was ID-only and args[0] was consumed)
      const categoryArg = message.mentions.channels.first() ? args[1] : args[1];
      ...
      setTicketSettings(message.guild!.id, {
        ...
        category_id: categoryArg ?? existing?.category_id,
        log_channel_id: existing?.log_channel_id,
        panel_message_id: panelMsg.id,
        enabled: true,
        custom_buttons: customBtns,
        ping_role_id: existing?.ping_role_id,
      });

      await message.reply({ embeds: [successEmbed('Ticket System Setup',
        `Ticket panel posted in <#${channel.id}>!\nMembers can open tickets using the buttons.` +
        (categoryArg ? `\n• **Category:** \`${categoryArg}\`\n\nTip: Use \`!ticketcategory <categoryID>\` anytime to update.` :
          `\n\n⚠️ No category set. Tickets will be created in the root of the server.\nUse \`!ticketcategory <categoryID>\` to assign a category.`)
      )] });
    }
  },
  ...
  // ── TICKET CATEGORY ───────────────────────────────────────────────────────
  {
    ...
    async execute(message, args) {
      ...
      if (!cat || cat.type !== ChannelType.GuildCategory)
        return message.reply({ embeds: [errorEmbed('Invalid Category', `\`${args[0]}\` is not a valid category ID.`)] });
      const settings = getTicketSettings(message.guild!.id);
      if (!settings) return message.reply({ embeds: [errorEmbed('Not Setup', 'Run `!ticketsetup` first.')] });
      updateTicketSettings(message.guild!.id, { category_id: args[0] });
      await message.reply({ embeds: [successEmbed('Category Set', `Tickets will now be created under **${cat.name}** (\`${args[0]}\`).`)] });
    }
  },
  ...
  {
    name: 'ticketlog',
    description: 'Set the channel for ticket open/close logs',
    category: 'Tickets',
    usage: 'ticketlog <#channel|channelID>',
    aliases: ['ticketlogs', 'setticketlog'],
    async execute(message, args) {
      if (!hasPermission(message.member!, Perms.ManageChannels))
        return message.reply({ embeds: [errorEmbed('No Permission', 'You need **Manage Channels** permission.')] });
      ...
      const channel = (message.mentions.channels.first() as TextChannel | undefined)
        ?? resolveTextChannel(message.guild!, args[0]);
      if (!channel)
        return message.reply({ embeds: [errorEmbed('Missing Channel', 'Mention the log channel or provide its **Channel ID**.\nExample: `!ticketlog #ticket-logs` or `!ticketlog 1234567890123456789`')] });

      const settings = getTicketSettings(message.guild!.id);
      if (!settings) return message.reply({ embeds: [errorEmbed('Not Setup', 'Run `!ticketsetup` first.')] });

      updateTicketSettings(message.guild!.id, { log_channel_id: channel.id });
      await message.reply({ embeds: [successEmbed('Ticket Log Set',
        `Ticket open/close activity will be logged in <#${channel.id}>.\n\n` +
        `ℹ️ Tip: You can also use \`!setlog ticketlog <#channel>\` to connect it to the global log system.`
      )] });
    }
  },
  ...
  // ── TICKET PING ROLE ──────────────────────────────────────────────────────
  {
    ...
    async execute(message, args) {
      ...
      await message.reply({ embeds: [successEmbed('Ticket Ping Role Set',
        `<@&${role.id}> will be **pinged** every time a new ticket is created.`
      )] });
    }
  },
  ...
  // ── TICKET VIEW ───────────────────────────────────────────────────────────
  {
    ...
    category: 'Tickets',
    usage: 'ticketview',
    aliases: ['ticketstatus', 'ticketconfig'],
    async execute(message) {
      if (!hasPermission(message.member!, Perms.ManageChannels))
        return message.reply({ embeds: [errorEmbed('No Permission', 'You need **Manage Channels** permission.')] });
      const s = getTicketSettings(message.guild!.id);
      if (!s) return message.reply({ embeds: [errorEmbed('Not Configured', 'Ticket system has not been set up yet.\nUse `!ticketsetup <#channel>` to start.')] });
      await message.reply({ embeds: [new EmbedBuilder()
        .setColor(COLORS.info)
        .setTitle('🎫 Ticket System Configuration')
        .addFields(
          { name: '✅ Status', value: s.enabled ? 'Enabled' : 'Disabled', inline: true },
          { name: '📢 Panel Channel', value: s.panel_channel_id ? `<#${s.panel_channel_id}>` : 'Not set', inline: true },
          { name: '📁 Category', value: s.category_id ? `<#${s.category_id}>` : '⚠️ Not set', inline: true },
          { name: '📋 Log Channel', value: s.log_channel_id ? `<#${s.log_channel_id}>` : '⚠️ Not set — use `!ticketlog <#ch>`', inline: true },
          { name: '🔔 Ping Role', value: s.ping_role_id ? `<@&${s.ping_role_id}>` : 'None', inline: true },
          { name: '🔘 Buttons', value: `${(s.custom_buttons ?? DEFAULT_BUTTONS).length} buttons configured`, inline: true },
        )
        .setFooter(BOT_FOOTER)
        .setTimestamp()
      ] });
