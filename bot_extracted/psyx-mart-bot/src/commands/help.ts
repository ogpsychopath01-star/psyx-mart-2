...
export const CATEGORY_META: Record<string, { emoji: string; description: string; color: number }> = {
  ...
  'Utility':        { emoji: '🔧',  description: 'Avatar, userinfo, ping, stats & more',                      color: 0x00D4FF },
  'Voice Bot':      { emoji: '🎙️', description: 'Join, leave, and 24/7 voice mode',                          color: 0x1ABC9C },
  'Logs':           { emoji: '📝',  description: 'Configure 14+ per-type logging channels',                   color: 0x5C6BC0 },
  'Automod':        { emoji: '🛡️', description: 'Antilink, antiraid, antispam, regprotect',                  color: 0xF39C12 },
  'Whitelist':      { emoji: '✅',  description: 'Whitelist trusted users, roles & channels from automod',    color: 0x2ECC71 },
  ...
  'Social':         { emoji: '🌐',  description: 'Fun commands — stain links, memes, games, text effects',   color: 0xFF6EC7 },
  'QR / Payment':   { emoji: '💳',  description: 'QR codes + UPI/bank payment info (3 payment slots)',       color: 0x00B4D8 },
  'Vouches':        { emoji: '📦',  description: 'Buyer vouches, seller ratings & vouch leaderboard',         color: 0x00E5CC },
  'Sell / Announce': { emoji: '🛍️', description: 'Secure product delivery, deal confirm, say & embed',      color: 0xFF8C00 },
...
function buildMainHelpEmbed(categories: string[], totalCmds: number) {
  const fields = categories.slice(0, 25).map(cat => {
...
const helpCommands: BotCommand[] = [
  {
    ...
    async execute(message, args) {
      ...
      const preferredOrder = [
        'Moderation', 'Mute', 'Percentage', 'Utility', 'Voice Bot',
        'Logs', 'Automod', 'Whitelist', 'Welcome', 'Temp VC', 'AFK',
        'Giveaway', 'Tickets', 'Setup', 'Jail', 'Server', 'Roles', 'Bio',
        'Social', 'QR / Payment', 'Vouches', 'Sell / Announce', 'Bot Owner',
      ];
