...
const setupCommands: BotCommand[] = [
  ...
  // ── TEMP ROLE ─────────────────────────────────────────────────────────────
  {
    ...
    async execute(message, args) {
      ...
      // Robustly find the duration: look for any arg that is a number (not a mention)
      const dayArg = args.find(a => !a.startsWith('<') && !isNaN(parseFloat(a)) && parseFloat(a) > 0);
      const days = dayArg ? parseFloat(dayArg) : NaN;
      ...
      if (isNaN(days) || days <= 0) return message.reply({ embeds: [errorEmbed('Invalid Days', 'Provide a valid positive number of days.\n**Example:** `!temprole @user @role 7`')] });
  ...
  // Staff can also set autoresponder for another user: !autoresponder set @user <message>
  {
    ...
    async execute(message, args) {
      ...
      if (sub === 'set') {
        ...
        await message.reply({ embeds: [successEmbed('Auto Responder Set', `When someone pings ${targetName}, PSYX MART will reply:\n> ${msg}`)] });
        ...
        await message.reply({ embeds: [new EmbedBuilder()
          ...
          .setDescription('When someone pings you, PSYX MART automatically sends your configured message.')
          .addFields(
  ...
  // Staff can also set autoreact for another user: !autoreact set @user <emoji>
  {
    ...
    async execute(message, args) {
      ...
      if (sub === 'set') {
        ...
        const targetName = mentionedUser && isStaff ? `<@${targetId}>` : 'you';
        await message.reply({ embeds: [successEmbed('Auto React Set', `When someone pings ${targetName}, PSYX MART will react with **${emojiArg}**`)] });
        ...
        await message.reply({ embeds: [new EmbedBuilder()
          .setColor(COLORS.info)
          .setTitle('⚡ Auto React')
          .setDescription('When someone pings you, PSYX MART reacts to their message with your chosen emoji.')
          .addFields(
