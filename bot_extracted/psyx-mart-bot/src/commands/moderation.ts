...
import { sendLog, resolveGuildMember, resolveMassTargets } from '../utils/helpers.js';
...
const moderation: BotCommand[] = [
  ...
  {
    name: 'ban',
    description: 'Ban a member from the server (mention or user ID)',
    category: 'Moderation',
    aliases: ['banuser'],
    ...
    async execute(message, args) {
      ...
      if (!target) return message.reply({ embeds: [errorEmbed('Missing User', 'Mention a member or provide their **User ID** to ban.')] });
      if (!canModerate(message.member!, target))
        return message.reply({ embeds: [errorEmbed('Hierarchy Error', 'You cannot ban this member.')] });
      const reason = args.slice(message.mentions.members!.size > 0 ? 1 : 1).join(' ') || 'No reason provided';
      try {
  ...
  {
    name: 'massban',
    description: 'Ban multiple members at once (mentions or user IDs)',
    category: 'Moderation',
    usage: 'massban <@user1|ID1> <@user2|ID2> ... [reason]',
    async execute(message, args) {
      if (!hasPermission(message.member!, Perms.BanMembers))
        return message.reply({ embeds: [errorEmbed('No Permission', 'You need **Ban Members** permission.')] });
      const targets = await resolveMassTargets(message, args);
      if (targets.size === 0) return message.reply({ embeds: [errorEmbed('Missing Users', 'Mention members or provide user IDs to ban.')] });
      const reason = args.filter(a => !/^(<@!?\d+>|\d{17,20})$/.test(a)).join(' ') || 'Mass ban';
      let banned = 0;
  ...
  {
    name: 'kick',
    description: 'Kick a member from the server (mention or user ID)',
    category: 'Moderation',
    ...
    async execute(message, args) {
      ...
      const target = await resolveGuildMember(message, args[0]);
      if (!target) return message.reply({ embeds: [errorEmbed('Missing User', 'Mention a member or provide their **User ID** to kick.')] });
      if (!canModerate(message.member!, target))
  ...
  {
    name: 'masskick',
    description: 'Kick multiple members at once (mentions or user IDs)',
    category: 'Moderation',
    usage: 'masskick <@user1|ID1> <@user2|ID2> ...',
    async execute(message, args) {
      if (!hasPermission(message.member!, Perms.KickMembers))
        return message.reply({ embeds: [errorEmbed('No Permission', 'You need **Kick Members** permission.')] });
      const targets = await resolveMassTargets(message, args);
      if (targets.size === 0) return message.reply({ embeds: [errorEmbed('Missing Users', 'Mention members or provide user IDs to kick.')] });
      const reason = args.filter(a => !/^(<@!?\d+>|\d{17,20})$/.test(a)).join(' ') || 'Mass kick';
      let kicked = 0;
      for (const [, member] of targets) {
        if (!canModerate(message.member!, member)) continue;
        try {
  ...
  {
    name: 'timeout',
    description: 'Timeout a member (mention or user ID)',
    category: 'Moderation',
    aliases: ['tm'],
    usage: 'timeout <@user|userID> <duration> [reason]  (e.g. 10m, 1h, 1d)',
    async execute(message, args) {
      ...
      const target = await resolveGuildMember(message, args[0]);
      if (!target) return message.reply({ embeds: [errorEmbed('Missing User', 'Mention a member or provide their **User ID**.')] });
      if (!canModerate(message.member!, target))
  ...
  {
    name: 'untimeout',
    description: 'Remove timeout from a member (mention or user ID)',
    category: 'Moderation',
    aliases: ['unmute', 'tmremove', 'removetm'],
    usage: 'untimeout <@user|userID>',
    async execute(message, args) {
      if (!hasPermission(message.member!, Perms.ModerateMembers))
        return message.reply({ embeds: [errorEmbed('No Permission', 'You need **Moderate Members** permission.')] });
      const target = await resolveGuildMember(message, args[0]);
  ...
  {
    name: 'masstm',
    description: 'Timeout multiple members (mentions or user IDs)',
    category: 'Moderation',
    usage: 'masstm <duration> <@user1|ID1> <@user2|ID2> ...',
    async execute(message, args) {
      if (!hasPermission(message.member!, Perms.ModerateMembers))
        return message.reply({ embeds: [errorEmbed('No Permission', 'You need **Moderate Members** permission.')] });
      const { parseTime } = await import('../utils/helpers.js');
      const durationStr = args[0];
      if (!durationStr) return message.reply({ embeds: [errorEmbed('Missing Duration', 'First arg must be a duration like `10m`, `1h`.')] });
      const ms = parseTime(durationStr);
      if (!ms) return message.reply({ embeds: [errorEmbed('Invalid Duration', `\`${durationStr}\` is not a valid duration. Use \`10m\`, \`1h\`, \`7d\`, etc.`)] });
      // Collect targets from both @mentions and raw IDs in args (skip args[0] = duration)
      const targets = await resolveMassTargets(message, args, true);
      if (targets.size === 0) return message.reply({ embeds: [errorEmbed('Missing Users', 'Mention members or provide user IDs to timeout.\n**Usage:** `!masstm 10m @user1 @user2` or `!masstm 1h 12345678901 12345678902`')] });
      let done = 0;
      for (const [, member] of targets) {
        if (!canModerate(message.member!, member)) continue;
        try {
          await dmUser(member, '⏰ You were Timed Out', `You have been **timed out** in **${message.guild!.name}** for **${durationStr}**.`);
          await member.timeout(ms); done++;
        } catch {}
      }
      await message.reply({ embeds: [modEmbed('⏰ Mass Timeout', `Timed out **${done}/${targets.size}** members for **${durationStr}**.`)] });
      await sendLog(message.client, message.guild!.id, 'memberslog', modEmbed('⏰ Mass Timeout', `**Mod:** ${message.author.tag}\n**Duration:** ${durationStr}\n**Count:** ${done} members`));
    }
  },
  ...
  {
    name: 'masstmremove',
    description: 'Remove timeout from multiple members (mentions or user IDs)',
    category: 'Moderation',
    usage: 'masstmremove <@user1|ID1> <@user2|ID2> ...',
    async execute(message, args) {
      if (!hasPermission(message.member!, Perms.ModerateMembers))
        return message.reply({ embeds: [errorEmbed('No Permission', 'You need **Moderate Members** permission.')] });
      const targets = await resolveMassTargets(message, args);
      if (targets.size === 0) return message.reply({ embeds: [errorEmbed('Missing Users', 'Mention members or provide user IDs.')] });
      let done = 0;
      for (const [, member] of targets) {
  ...
  {
    name: 'warn',
    description: 'Warn a member (mention or user ID)',
    category: 'Moderation',
  ...
  {
    name: 'masswarn',
    description: 'Warn multiple members at once (mentions or user IDs)',
    category: 'Moderation',
    usage: 'masswarn <@user1|ID1> <@user2|ID2> ... [reason]',
    async execute(message, args) {
      if (!hasPermission(message.member!, Perms.ModerateMembers))
        return message.reply({ embeds: [errorEmbed('No Permission', 'You need **Moderate Members** permission.')] });
      const targets = await resolveMassTargets(message, args);
      if (targets.size === 0) return message.reply({ embeds: [errorEmbed('Missing Users', 'Mention members or provide user IDs.')] });
      const reason = args.filter(a => !/^(<@!?\d+>|\d{17,20})$/.test(a)).join(' ') || 'Mass warn';
      for (const [, member] of targets) {
        if (!canModerate(message.member!, member)) continue;
  ...
  {
    name: 'warnings',
    description: 'Check warnings of a user (mention or user ID)',
    category: 'Moderation',
    ...
    async execute(message, args) {
      if (!hasPermission(message.member!, Perms.ModerateMembers))
        return message.reply({ embeds: [errorEmbed('No Permission', 'You need **Moderate Members** permission.')] });
      const target = args[0] ? await resolveGuildMember(message, args[0]) : message.member!;
      if (!target) return message.reply({ embeds: [errorEmbed('Not Found', 'Could not find that member.')] });
      const warns = getWarnings(message.guild!.id, target.id);
  ...
  {
    name: 'removewarn',
    description: 'Remove warnings from a user (mention or user ID)',
    category: 'Moderation',
    aliases: ['clearwarns', 'delwarn'],
    usage: 'removewarn <@user|userID> [count]',
    async execute(message, args) {
      if (!hasPermission(message.member!, Perms.ModerateMembers))
        return message.reply({ embeds: [errorEmbed('No Permission', 'You need **Moderate Members** permission.')] });
      const target = await resolveGuildMember(message, args[0]);
  ...
  // ── PURGE ─────────────────────────────────────────────────────────────────
  {
    ...
    category: 'Moderation',
    aliases: ['clear', 'prune'],
    usage: 'purge <amount> [@user|userID]',
    async execute(message, args) {
      ...
      // Resolve optional filter user (mention or ID)
      const filterMember = args[1] ? await resolveGuildMember(message, args[1]) : null;
      const filterUserId = filterMember?.id ?? (args[1] && /^\d{17,20}$/.test(args[1]) ? args[1] : null);
      ...
      try {
        ...
        while (deleted < amount) {
          ...
          const fetched: any = await (message.channel as TextChannel).messages.fetch(fetchOptions as any);
          if (!fetched.size) break;
          lastId = fetched.last()?.id;
          let batch = [...fetched.values()];
          if (filterUserId) batch = batch.filter((m: any) => m.author.id === filterUserId);
  ...
  // ── WARNINGS LEADERBOARD ─────────────────────────────────────────────────
  {
    ...
    async execute(message, args) {
      ...
      if (!allWarns.length) return message.reply({ embeds: [new EmbedBuilder()
        ...
        .setFooter({ text: `✦ PSYX MART Bot • Owner: ogpsyx` })
        .setTimestamp()
      ] });
      ...
      await message.reply({ embeds: [new EmbedBuilder()
        ...
        .setFooter({ text: `Showing top ${sorted.length} • ✦ PSYX MART Bot • Owner: ogpsyx` })
        .setTimestamp()
      ] });
    }
  },
