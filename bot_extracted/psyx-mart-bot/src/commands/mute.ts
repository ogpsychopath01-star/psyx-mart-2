...
import { sendLog, resolveGuildMember } from '../utils/helpers.js';
...
const muteCommands: BotCommand[] = [
  ...
  {
    name: 'chatmute',
    description: 'Mute a user so they cannot chat (mention or user ID)',
    category: 'Mute',
    usage: 'chatmute <@user|userID> [reason]',
    async execute(message, args) {
      ...
      try {
        ...
        await sendLog(message.client, message.guild!.id, 'memberslog', modEmbed('🔇 Chat Mute', `**User:** ${target.user.tag}\n**Mod:** ${message.author.tag}\n**Reason:** ${reason}`));
  ...
  {
    name: 'chatunmute',
    description: 'Unmute a user (mention or user ID)',
    category: 'Mute',
    usage: 'chatunmute <@user|userID>',
    async execute(message, args) {
      if (!hasPermission(message.member!, Perms.ManageRoles))
        return message.reply({ embeds: [errorEmbed('No Permission', 'You need **Manage Roles** permission.')] });
      const target = await resolveGuildMember(message, args[0]);
  ...
  {
    name: 'vcmute',
    description: 'Server mute a member in voice channel (mention or user ID)',
    category: 'Mute',
    usage: 'vcmute <@user|userID>',
    async execute(message, args) {
  ...
  {
    name: 'vcunmute',
    description: 'Remove server mute from a member (mention or user ID)',
    category: 'Mute',
    usage: 'vcunmute <@user|userID>',
    async execute(message, args) {
      if (!hasPermission(message.member!, Perms.MuteMembers))
        return message.reply({ embeds: [errorEmbed('No Permission', 'You need **Mute Members** permission.')] });
      const target = await resolveGuildMember(message, args[0]);
  ...
  {
    name: 'vcdeafen',
    description: 'Server deafen a member (mention or user ID)',
    category: 'Mute',
    usage: 'vcdeafen <@user|userID>',
    async execute(message, args) {
  ...
  {
    name: 'vcundeafen',
    description: 'Undeafen a member in voice channel (mention or user ID)',
    category: 'Mute',
    usage: 'vcundeafen <@user|userID>',
    async execute(message, args) {
      if (!hasPermission(message.member!, Perms.DeafenMembers))
        return message.reply({ embeds: [errorEmbed('No Permission', 'You need **Deafen Members** permission.')] });
      const target = await resolveGuildMember(message, args[0]);
  ...
  {
    name: 'vcmove',
    description: 'Move a member to another voice channel (mention or user ID, channel ID)',
    category: 'Mute',
    usage: 'vcmove <@user|userID> <channelID>',
    async execute(message, args) {
  ...
  {
    name: 'vcmoveall',
    description: 'Move all members in your VC to another channel (channel ID)',
    category: 'Mute',
  ...
  {
    name: 'vckick',
    description: 'Kick a member from their voice channel (mention or user ID)',
    category: 'Mute',
  ...
  {
    name: 'vcban',
    description: 'Ban a user from joining any voice channel (mention or user ID)',
    category: 'Mute',
    usage: 'vcban <@user|userID> [reason]',
    async execute(message, args) {
      if (!hasPermission(message.member!, Perms.MuteMembers))
        return message.reply({ embeds: [errorEmbed('No Permission', 'You need **Mute Members** permission.')] });
      const target = await resolveGuildMember(message, args[0]);
  ...
  {
    name: 'vcunban',
    description: 'Unban a user from voice channels (mention or user ID)',
    category: 'Mute',
    usage: 'vcunban <@user|userID>',
    async execute(message, args) {
      if (!hasPermission(message.member!, Perms.MuteMembers))
        return message.reply({ embeds: [errorEmbed('No Permission', 'You need **Mute Members** permission.')] });
      const target = await resolveGuildMember(message, args[0]);
  ...
  {
    name: 'vchide',
    description: 'Hide a voice channel from everyone (channel ID or be in a VC)',
    category: 'Mute',
  ...
  {
    name: 'vcunhide',
    description: 'Unhide a voice channel (channel ID or be in a VC)',
    category: 'Mute',
