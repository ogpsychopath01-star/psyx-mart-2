  ...
  // ── BOTOWNER INFO ─────────────────────────────────────────────────────────
  {
    ...
    async execute(message) {
      ...
      const embed = new EmbedBuilder()
        .setColor(0xFFD700)
        .setTitle('👑  Bot Owner — ogpsyx')
        .setDescription(`<@${BOT_OWNER_ID}> is the creator and owner of **PSYX MART Bot**.\n\nResponsible for all features, updates, and support.`)
        .setTimestamp();
      if (owner) {
        embed.setThumbnail(owner.displayAvatarURL({ size: 256 }));
        embed.addFields(
          { name: '🏷️ Username', value: owner.tag, inline: true },
          { name: '🌐 Servers', value: `${message.client.guilds.cache.size}`, inline: true },
          { name: '📅 On Discord Since', value: `<t:${Math.floor(owner.createdTimestamp / 1000)}:D>`, inline: true },
        );
      }
  ...
  // ── POLL ──────────────────────────────────────────────────────────────────
  {
    ...
    async execute(message, args) {
      ...
      const embed = new EmbedBuilder()
        ...
        .setTimestamp();
      const poll = await (message.channel as any).send({ embeds: [embed] });
      await poll.react('✅');
      await poll.react('❌');
    ...
    async execute(message, args) {
      ...
      setTimeout(async () => {
        try {
          ...
          await (message.channel as any).send({ content: `<@${message.author.id}>`, embeds: [new EmbedBuilder()
